...
static inline int spl_array_object_verify_pos(spl_array_object *object, HashTable *ht, int check_current)
{
	if (!ht) {
		...
		if (object->pos == NULL) {
			if (check_current) {
				if (object->pos_h == 0 && ht->pInternalPointer == NULL) {
					php_error_docref(NULL TSRMLS_CC, E_NOTICE, "Array was empty. No current entry available");
				} else {
					php_error_docref(NULL TSRMLS_CC, E_NOTICE, "Array reached the end. No current entry available");
				}
			}

			return FAILURE;
		} else if (spl_hash_verify_pos_ex(object, ht TSRMLS_CC) == FAILURE) {
...
SPL_METHOD(Array, current)
{
	...
	if (spl_array_object_verify_pos(intern, aht, 1) == FAILURE) {
		return;
	}
...
SPL_METHOD(Array, next)
{
	...
	if (spl_array_object_verify_pos(intern, aht, 0) == FAILURE) {
		return;
	}
